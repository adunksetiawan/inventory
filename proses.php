	...
	switch($proses){
		...
		case "barang_insert":
            ...
            case "kategori_insert":
			{
                ...
                $ckat=mysql_query("select kode_kat from barang_kategori where kode_kat='$_POST[kode_kat]'");
                $dtkat=mysql_fetch_array($ckat);
                
                $values="NULL, '$_POST[kode_kat]', '$_POST[nm_kat]'";
                $hal="data_barang";
                insert($nama_tabel,$values);
                break;
            }
		...
		case "beli_insert":
		{
			...
			while($dtmp=mysql_fetch_array($qtmp))
			{
				...
				$beli_detail="INSERT INTO beli_detail(beli_id, barang_id, barang_nama, kategori, qty, packing, harga_satuan, harga_total)
				VALUES('$dtmp[beli_id]', '$dtmp[barang_id]', '$dtmp[barang_nama]', '$dtmp[kategori]', '$dtmp[qty]', 
				'$dtmp[packing]', '$dtmp[harga_satuan]', '$dtmp[harga_total]')";
				...
				else
				{
					...
					{
						//insert data
						$stok="INSERT INTO stok(barang_id, barang_nama, kategori, qty, packing)
		...
		case "jual_insert":
		{
			...
			if($sisa_piutang!=0)
			{
				...
				'$sisa_piutang', '$_POST[jml_bayar]', '$sisa_piutang', '$tgl_jatuh_tempo', 'blm lunas')";
				mysql_query($piutang);
